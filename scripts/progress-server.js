// Live progress dashboard for the self-improve / app-improve loops.
...
function state() {
  // pick whichever backlog is the active one (app loop vs data loop) by last log mode
  const logFull = rd('SELF-IMPROVE-LOG.md').split('\n').filter(Boolean);
  let start = 0;
  for (let i = logFull.length - 1; i >= 0; i--) if (/\] mode:/.test(logFull[i])) { start = i; break; }
  const run = logFull.slice(start);
  const runText = run.join('\n');
  const isApp = /APP IMPROVE/.test(runText);
  ...
  for (let i = run.length - 1; i >= 0; i--) {
    ...
    if (phase === 'idle') {
      ...
      if (/\] model:/.test(l) || /picked:/.test(l)) { phase = 'building'; break; }
    }
  }
  const finished = /batch complete/.test(runText);
  const tail = logFull.slice(-12);
  let elapsed = null;
  ...
  return { done, pending, total, committed, rejected, modeLine, cur, batchN, item, phase, finished, tail, elapsed };
}

const elapsedStr = s => s == null ? '—' : s < 60 ? s + 's' : Math.floor(s / 60) + 'm ' + (s % 60) + 's';
...
function page() {
  ...
  const phaseColor = { building:'#00b4ff', reviewed:'#ffd60a', committed:'#00ff88', rejected:'#ff2d55', done:'#00ff88', idle:'#a0b8d8' }[s.phase] || '#e8f0ff';
  const esc = t => (t || '').replace(/[<>&]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]));
  const working = !s.finished && s.phase !== 'idle';
  const stuck = s.elapsed != null && s.elapsed > 1500;
...
<style>
 ...
 .g{color:#00ff88}.r{color:#ff2d55}.b{color:#00b4ff}
 ...
 <div class="lbl">Now: <span class="phase" style="color:${phaseColor}">${s.finished?'BATCH COMPLETE':s.phase.toUpperCase()}</span>
   ${working?`<span style="color:${stuck?'#ffd60a':'#a0b8d8'};font-size:12px;margin-left:8px">⚙ working · ${elapsedStr(s.elapsed)} since last event${stuck?' ⚠ may be stuck':''}</span>`:''}</div>
